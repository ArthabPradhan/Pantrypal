...
const appJsPath = 'C:\\\\Users\\\\NITRO\\\\Documents\\\\HW App\\\\PantryPal\\\\App.js';
let appJsContent = fs.readFileSync(appJsPath, 'utf-8');

const startMarker = '      setRecipes([';
const startIndex = appJsContent.indexOf(startMarker);

if (startIndex !== -1) {
    const endIndex = appJsContent.indexOf('      ]);', startIndex) + '      ]);'.length;
    const replacementStr = '      setRecipes(' + JSON.stringify(recipes, null, 8) + ');';
    const newContent = appJsContent.substring(0, startIndex) + replacementStr + appJsContent.substring(endIndex);
    fs.writeFileSync(appJsPath, newContent, 'utf-8');
    console.log('App.js updated');
} else {
    // If it was already replaced without newlines, try another match
    console.log('Fallback logic, trying broader match for App.js');
    const rx = /setRecipes\(\[.*?\]\);/s;
    if (rx.test(appJsContent)) {
        let newContent = appJsContent.replace(rx, 'setRecipes(' + JSON.stringify(recipes, null, 8) + ');');
        fs.writeFileSync(appJsPath, newContent, 'utf-8');
        console.log('App.js updated via RegExp');
    }
}

const appHtmlPath = 'C:\\\\Users\\\\NITRO\\\\Documents\\\\HW App\\\\PantryPal\\\\app.html';
let appHtmlContent = fs.readFileSync(appHtmlPath, 'utf-8');

let cardsHtml = '';
recipes.forEach((r, idx) => {
    let tagClass = 'dtag';
    if (r.diet === 'Vegan') tagClass = 'dtag veg';
    if (r.diet === 'Non-veg') tagClass = 'dtag veg'; // reusing class for style

    const bgMap = ['g', 'a', 'p'];
    const bg = bgMap[idx % 3];

    cardsHtml += `    <div class="rcard" onclick="show('detail')">
      ...
      <div class="rinfo">
        ...
        <div class="rmeta">
          ...
          <span class="${tagClass}">${r.diet}</span>
...
const startMarkerHtml = '<div class="ctx"><span>Based on: rice · egg · tomato</span></div>\\n';
const startIndexHtml = appHtmlContent.indexOf('<div class="ctx"><span>Based on: rice · egg · tomato</span></div>\n');

if (startIndexHtml !== -1) {
    const endIndexHtml = appHtmlContent.indexOf('    <div class="bnav">', startIndexHtml);
    const newContentHtml = appHtmlContent.substring(0, startIndexHtml + '<div class="ctx"><span>Based on: rice · egg · tomato</span></div>\n'.length) + cardsHtml + appHtmlContent.substring(endIndexHtml);
    fs.writeFileSync(appHtmlPath, newContentHtml, 'utf-8');
    console.log('app.html updated');
} else {
    // If we can't find it, the script may have already run or been modified
    console.log('Fallback logic, trying broader match for app.html');
    const rxHtml = /(<div class="ctx"><span>Based on: rice · egg · tomato<\/span><\/div>\n)(.*?)(    <div class="bnav">)/s;
    if (rxHtml.test(appHtmlContent)) {
        let newContentHtml = appHtmlContent.replace(rxHtml, `$1${cardsHtml}$3`);
        fs.writeFileSync(appHtmlPath, newContentHtml, 'utf-8');
        console.log('app.html updated via RegExp');
    } else {
        console.log('Could not find section in app.html to replace');
    }
}
